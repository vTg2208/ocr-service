...
class FRAWorkspaceUITests(unittest.TestCase):
    ...
    def test_administrative_filters_are_collapsed_into_the_active_workspace_header(self):
        ...
        self.assertEqual(html.count('class="workspace-heading-actions filter-host"'), 7)
        self.assertNotIn('class="overview-refresh filter-host"', html)
        self.assertIn('<h1 id="dashboardTitle">Dashboard</h1>', html)
        self.assertIn('id="refreshDashboard" class="text-action refresh-action"', html)
        self.assertIn('src="/static/fra/icons/refresh.png"', html)
        self.assertTrue((UI_ROOT / "icons" / "refresh.png").is_file())
        self.assertIn('<span>Refresh</span></button>', html)
